...
class TestConsole(unittest.TestCase):
    ...
    @unittest.skipIf(getenv("HBNB_TYPE_STORAGE") == "db", "can't run if storage is db")
    ...
    def test_create_v2(self):
        ...
        with patch("sys.stdout", new=StringIO()) as f:
            self.consol.onecmd(
                'create City name="San_Francisco state_id="{}"'.format(id)
            )
            id = f.getvalue()[:-1]
            self.assertEqual(len(id), 36)
        with patch("sys.stdout", new=StringIO()) as f:
    ...
    def test_create_v2_params(self):
        ...
        with patch("sys.stdout", new=StringIO()) as f:
            self.consol.onecmd(
                'create User email="ilovetim@google.com"\
                               password="timisboss"\
